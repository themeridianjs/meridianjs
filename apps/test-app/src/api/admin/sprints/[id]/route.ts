...
export const PUT = async (req: any, res: Response) => {
  ...
  // Completing a sprint goes through the compensatable workflow
  if (status === "completed") {
    const { result: sprint, errors, transaction_status } = await completeSprintWorkflow(req.scope).run({
      input: {
        sprintId: req.params.id,
        moveIncompleteToSprintId: req.body.moveIncompleteToSprintId ?? null,
  ...
  // Regular field updates (rename, reschedule, etc.) — direct service call
  ...
  if (status !== undefined) updates.status = status // "planned" or "active"
