...
export const createInvitationWorkflow = createWorkflow(
  ...
  async (input: CreateInvitationInput) => {
    ...
    // Emit a global event so any listener (email sender, audit log, etc.) can react.
    // The token is all a listener needs to construct the invite URL for their environment.
    await emitEventStep({
