...
const setAssigneesStep = createStep(
  "set-issue-assignees",
  async (input: { issue: any; newAssignees: string[]; actor_id?: string | null }, { container }) => {
    const svc = container.resolve("issueModuleService") as any
    const updated = await svc.updateIssue(input.issue.id, { assignee_ids: input.newAssignees })
    return new StepResponse(updated, { issueId: input.issue.id, previousAssignees: input.issue.assignee_ids ?? [] })
  },
...
const logAssigneesStep = createStep(
  "log-issue-assignees",
  async (
    input: { entity_id: string; actor_id: string; workspace_id: string; assignee_ids: string[]; oldAssignees: string[] },
    { container }
  ) => {
    ...
    await svc.recordActivity({
      ...
      actor_id: input.actor_id, action, workspace_id: input.workspace_id,
...
export const assignIssueWorkflow = createWorkflow(
  ...
  async (input: AssignIssueInput) => {
    ...
    const activityInput = transform(updated, (u) => ({
      ...
      workspace_id: u.workspace_id, assignee_ids: newAssignees, oldAssignees,
    }))
    await logAssigneesStep(activityInput)
    await emitEventStep({
      name: EVENTS.ISSUE_ASSIGNED,
