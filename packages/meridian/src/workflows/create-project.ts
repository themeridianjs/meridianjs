...
const validateIdentifierStep = createStep(
  ...
  async (input: CreateProjectInput, { container }) => {
    ...
    if (existing) {
      throw Object.assign(new Error(`Identifier "${identifier}" is already taken`), { status: 409 })
    }
...
const createProjectStep = createStep(
  "create-project",
  async (input: CreateProjectInput & { identifier: string }, { container }) => {
    const svc = container.resolve("projectModuleService") as any
...
const seedDefaultStatusesStep = createStep(
  ...
  async (input: { project_id: string; initial_statuses?: InitialStatus[] }, { container }) => {
    const svc = container.resolve("projectModuleService") as any
  ...
  async ({ statusIds }: { statusIds: string[] }, { container }) => {
    const svc = container.resolve("projectModuleService") as any
    for (const id of statusIds) await svc.deleteProjectStatus(id)
  }
)
...
const logProjectCreatedStep = createStep(
  ...
  async (input: { entity_id: string; actor_id: string; workspace_id: string }, { container }) => {
    ...
    await svc.recordActivity({
      entity_type: "project", entity_id: input.entity_id,
...
export const createProjectWorkflow = createWorkflow(
  ...
  async (input: CreateProjectInput) => {
    ...
    const seedInput = transform(project, (p) => ({
      project_id: p.id, initial_statuses: input.initial_statuses,
    }))
    await seedDefaultStatusesStep(seedInput)
    const activityInput = transform(project, (p) => ({
      entity_id: p.id, actor_id: input.actor_id ?? "system", workspace_id: p.workspace_id,
    }))
    await logProjectCreatedStep(activityInput)
    await emitEventStep({
      name: "project.created",
      data: { project_id: project.id, workspace_id: project.workspace_id, actor_id: input.actor_id ?? "system" },
    })
