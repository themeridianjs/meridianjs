...
/**
 * Method signatures generated by {@link MeridianService} for a model name —
 * use to type call sites, e.g. `resolveService<GeneratedCrudMethods<"Project">>(...)`.
 * Params/returns stay `any` (rows are schema-defined at runtime); the value is
 * method-name + arity safety.
 *
 * NOT intersected into the factory's return type: mapped-type members are
 * properties, and TS2425 forbids subclasses overriding a base property with a
 * method — which module services legitimately do (e.g. notification's
 * createNotification).
 */
export type GeneratedCrudMethods<M extends string> = {
  [K in M as `list${K}s`]: (filters?: any, options?: any) => Promise<any[]>
} & {
  [K in M as `listAndCount${K}s`]: (filters?: any, options?: any) => Promise<[any[], number]>
} & {
  [K in M as `retrieve${K}`]: (id: string, options?: any) => Promise<any>
} & {
  [K in M as `create${K}`]: (data: any) => Promise<any>
} & {
  [K in M as `update${K}`]: (id: string, data: any) => Promise<any>
} & {
  [K in M as `delete${K}`]: (id: string) => Promise<void>
} & {
  [K in M as `softDelete${K}`]: (id: string) => Promise<any>
}
 ...
 */
/**
 * Instance type returned by the {@link MeridianService} factory. The index
 * signature is intentionally loose HERE (generated method names are runtime
 * data — see GeneratedCrudMethods for why they can't be static members); the
 * public `IModuleService` in @meridianjs/types stays `unknown`-indexed.
 */
export interface MeridianServiceInstance extends IModuleService {
  [method: string]: any
}

export function MeridianService<T extends Record<string, ModelDefinition>>(
  models: T
): new (container: MeridianContainer) => MeridianServiceInstance {
